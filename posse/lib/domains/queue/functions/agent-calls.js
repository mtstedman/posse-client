...
export function createAgentCall({
  ...
  max_output_tokens_configured = null,
  provider = "claude",
...
export function completeAgentCall(id, {
  ...
  db.prepare(`
    ...
    SET status = ?, finished_at = ?, duration_ms = ?,
        ...
        max_output_tokens_configured = COALESCE(max_output_tokens_configured, ?),
        output_truncated = ?,
        output_limit_reason = ?,
  ...
  `).run(
    ...
    max_output_tokens_configured,
    output_truncated ? 1 : 0,
    output_limit_reason,
    model_name,
    exit_code, error_text,
    atlas_method,
