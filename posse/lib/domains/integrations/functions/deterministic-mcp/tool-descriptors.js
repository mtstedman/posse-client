...
export const TOOL_EXECUTION_SPECS = Object.freeze({
  ...
  "fetch_ref": { access: "atlas", summary: "Traverse one or more opaque refs from the current agent scope. A ref or returned next-page cursor opens an already-stored result; it is not a fresh retrieval and does not rerun the originating tool. During exploration, fetch only omitted/bounded content, cursor or survey refs, or focused stored-result matches; auto mode supports literal and regex/OR syntax. The visible-content ledger rejects fully emitted content, duplicate pages, and repeated empty searches. Batch known refs, and call the originating tool again only for a materially different scope. In bounded researcher runs, closeout admits one final multi-ref batch, then requires synthesis." },
