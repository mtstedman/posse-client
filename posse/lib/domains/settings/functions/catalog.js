...
export const SETTINGS_CATALOG = [
  ...
  { key: "atlas_search_result_paging", default: "off",            options: ["off", "on"], description: "Bounded-ingress paging for symbol.search results: keep the top of the ranked payload inline (10K char cap) and page the remainder behind fetch_ref. Off (default) leaves symbol.search unbounded." },
  { key: "atlas_result_ref_paging",       default: "off",                options: ["off", "on"], description: "Window/lens result ref-paging (TOKEN-LEVERS L3b): when a code.window/code.lens result exceeds atlas_result_ref_paging_min_chars, keep the top-ranked portion inline (code.lens: top matches; code.window: head lines) and page the remainder behind a fetch_ref stub. Off (default) leaves window/lens results inline." },
  ...
  { key: "atlas_prefetch_entrypoint_rank", default: "off", options: ["off", "on"], description: "Entry-point/export-graph prefetch ranking (TOKEN-LEVERS L4a): promote entry-shaped and strongly imported files in native tree.scope candidate order. Off (default) preserves current ranking." },
  ...
  { key: "atlas_survey_tail_refs",        default: "off",                options: ["off", "on"], description: "code.survey tail compaction: keep the top-ranked files' skeletons inline and page lower-ranked files' skeletons behind a fetch_ref payload. Off (default) leaves survey results uncompacted." },
  { key: "atlas_gate_nudge",              default: "off",                options: ["off", "on"], description: "Token-pressure ladder steering (TOKEN-LEVERS L3a): when per-target lens/window thresholds trip, append the redirect recommendation in-band to the triggering ATLAS result. Off (default) keeps the detector observation-only (shadow)." },
  ...
  { key: "atlas_gateway_dedup_advertise", default: "off",                options: ["off", "on"], description: "Gateway de-advertisement (TOKEN-LEVERS L5a): on the owner-hot gateway path, drop the four ATLAS gateway wrappers (query/code/repo/agent) from the advertised tools/list since the individual per-action tools are already advertised. Dispatch is untouched — remote-issued gateway calls still route. Off (default) advertises the gateways." },
  { key: "atlas_prose_dedup",             default: "off",                options: ["off", "on"], description: "Policy-prose dedup (TOKEN-LEVERS L5b): the runtime role contract emits a compact single-statement fallback/anti-fabrication policy instead of the full closing block, since the handoff atlas-context prose already delivers the full retrieval policy. Off (default) keeps the full policy in the role contract." },
