...
import {
  ...
  RESEARCH_EVIDENCE_REUSE_MODE_VALUES,
...
export const SETTINGS_CATALOG = [
  ...
  // Session owns trunk enrollment and publication. Keep these repo-scoped
  // runtime/legacy CLI keys for existing and manually configured trunks, but
  // omit them from the operator admin description consumed by Posse and Bossy.
  { key: SETTING_KEYS.SHARED_TRUNK_ENABLED, default: String(SHARED_TRUNK_DEFAULTS.enabled), scope: "repo", adminVisible: false, valueType: "boolean", description: "Allow this clone to synchronize and automatically push its configured Posse side trunk" },
  { key: SETTING_KEYS.SHARED_TRUNK_BRANCH, default: SHARED_TRUNK_DEFAULTS.branch, scope: "repo", adminVisible: false, description: "Shared Posse side-trunk branch; must match target_branch and must not be the remote default branch" },
  { key: SETTING_KEYS.SHARED_TRUNK_REMOTE, default: SHARED_TRUNK_DEFAULTS.remote, scope: "repo", adminVisible: false, description: "Git remote used to synchronize the shared Posse side trunk" },
  { key: SETTING_KEYS.SHARED_TRUNK_FETCH_INTERVAL_SEC, default: String(SHARED_TRUNK_DEFAULTS.fetchIntervalSec), scope: "repo", adminVisible: false, numeric: { integer: true, ...SHARED_TRUNK_LIMITS.fetchIntervalSec }, description: "Seconds between shared-trunk fetches while a scheduler run has active or queued work" },
  { key: SETTING_KEYS.SHARED_TRUNK_FETCH_INTERVAL_IDLE_SEC, default: String(SHARED_TRUNK_DEFAULTS.fetchIntervalIdleSec), scope: "repo", adminVisible: false, numeric: { integer: true, ...SHARED_TRUNK_LIMITS.fetchIntervalIdleSec }, description: "Seconds between shared-trunk fetches while a live scheduler run is idle-parked" },
  { key: SETTING_KEYS.SHARED_TRUNK_PUSH_RETRY_MAX, default: String(SHARED_TRUNK_DEFAULTS.pushRetryMax), scope: "repo", adminVisible: false, numeric: { integer: true, ...SHARED_TRUNK_LIMITS.pushRetryMax }, description: "Maximum bounded fetch/re-merge retries after a shared-trunk push race" },
  { key: SETTING_KEYS.SHARED_TRUNK_CLAIMS_ENABLED, default: String(SHARED_TRUNK_DEFAULTS.claimsEnabled), scope: "repo", adminVisible: false, valueType: "boolean", description: "Publish and consume advisory cross-instance file claims for this shared trunk" },
  { key: SETTING_KEYS.SHARED_TRUNK_CLAIMS_TTL_MIN, default: String(SHARED_TRUNK_DEFAULTS.claimsTtlMin), scope: "repo", adminVisible: false, numeric: { integer: true, ...SHARED_TRUNK_LIMITS.claimsTtlMin }, description: "Maximum lifetime in minutes of an advisory cross-instance file claim" },
  { key: SETTING_KEYS.SHARED_TRUNK_CLAIM_DEFER_MAX_MIN, default: String(SHARED_TRUNK_DEFAULTS.claimDeferMaxMin), scope: "repo", adminVisible: false, numeric: { integer: true, ...SHARED_TRUNK_LIMITS.claimDeferMaxMin }, description: "Maximum minutes a peer claim may defer dispatch before the advisory hardens open" },
  ...
  { key: "posse_max_fix_chain_depth", default: "2", numeric: { integer: true, min: 1 }, description: "Maximum fix-chain depth (fix→fix→fix…) before automatically researching and replanning" },
  ...
  { key: "research_traversal_completion_max_chars", default: "1600", numeric: { integer: true, min: 180 }, description: "Maximum rendered characters for the traversal completion check directive" },
  { key: SETTING_KEYS.RESEARCH_SYNTHESIS_MAX_PHYSICAL_CALLS, default: "", numeric: { integer: true, min: 16, max: 200 }, description: "Experimental researcher physical tool-call ceiling per attempt (blank keeps the built-in 26). Logical exploration units, batch width, turn, and novelty limits are unchanged; the value is snapshotted once per research session" },
  { key: SETTING_KEYS.ATLAS_RESEARCH_RUNTIME_GUIDANCE, default: "on", options: ["off", "on"], adminVisible: false, description: "Model-visible dynamic Atlas research notices. Off preserves enforcement, accounting, result metadata, and ordinary error responses without appending navigation, repair, batching, or closeout instructions." },
  ...
  { key: "atlas_researcher_schema_diet", default: "off",                options: ["off", "on"], adminVisible: false, description: "Experimental researcher-only provider schema diet: use concise top-level semantics, description-free input fields, and no advertised fetch_ref compatibility alias while preserving runtime validation, canonical dispatch, and all JSON Schema constraints." },
  ...
  { key: "atlas_handoff_prefetch_timeout_ms", default: "90000",          numeric: { integer: true, min: 1000 }, adminVisible: false, description: "Maximum milliseconds to wait for ATLAS handoff prefetch before deterministic fallback (default 90s — covers the composed tree and survey prefetch under contention)" },
