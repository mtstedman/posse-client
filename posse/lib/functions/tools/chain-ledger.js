...
export function createChainLedger({
  ...
  function chainRead(args = {}) {
    ...
    const limit = normalizeReadRange(args.limit, 2000);
    ...
    const result = readFile({ ...args, path: requestedPath }, workspaceCwd, scopePredicates);
  ...
  return { chainRead, chainVerdict, state };
}
